...
function Favorite(){
    ...
    useEffect(()=>{
        ...
        if(storeCryptos){
          setListaCryptosLikes(storeCryptos);
        }
    }
    ,[])
    useEffect(() => {
    ...
    return (
        <>
            ...
            <main className='inicioContainer'>
                <table className='cryptos-favoritas'>
                    <thead className='encabezado'>
                        <tr>
                            <th></th>
                            <th style={{width:'15%',textAlign:'start'}}>#</th>
                            <th style={{textAlign:'start'}}>Coin</th>
                            <th>Remove</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        listaCryptosLikes.map((c,index)=>{
                            return(
                                <tr>
                                    ...
                                    <td className='name-C' onClick={()=>redireccionar(c.ID)}>
                                        <img src={c.IMAGE} alt="" />
                                        <span className='name'>{c.NAME}</span>
                                        <span className='symbol'>{c.SYMBOL}</span>
                                    </td>
                                    <td>
                                        <button class="tooltip" onClick={()=>removeCrypto(c.ID)}>
